...
export default function ProblemPage() {
  ...
  const {review,status:reviewStatus,error:reviewError,request:requestReview}=useCodeReview(detail?.problem ?? null,selected);
  ...
  return <div className="appShell"><SiteHeader/><main className="detailMain">
    ...
    {detail?<>
      ...
      {selected?.code?<section className="workspace">
        ...
        <div className="reviewCallout"><div><strong>이 풀이를 한 번 더 점검해 볼까요?</strong><span>버튼을 누르면 AI 리뷰를 요청합니다.</span></div><button className="reviewTrigger" type="button" disabled={reviewStatus==='loading'} onClick={()=>void requestReview(Boolean(review))}>{reviewStatus==='loading'?'코드 분석 중…':review?'AI 리뷰 다시 받기':'AI 리뷰 받기'} <span>→</span></button></div>
        ...
        <section className="aiReview">
          ...
          {reviewStatus==='error'?<div className="reviewFallback"><strong>AI 리뷰를 완료하지 못했습니다.</strong><p>{reviewError}</p><button className="primaryButton" type="button" onClick={()=>void requestReview(true)}>다시 요청하기</button></div>:null}
          {reviewStatus==='loading'?<div className="reviewSkeleton" role="status" aria-live="polite"><span className="visuallyHidden">AI 코드 리뷰를 생성하고 있습니다.</span><div className="skeletonSummary"><i className="skLabel"/><i className="skTitle"/><i className="skText medium"/></div><div className="skeletonContext"><div><i className="skLabel"/><i className="skText"/><i className="skText short"/></div><div><i className="skLabel"/><i className="skText medium"/><i className="skText short"/></div></div><div className="skeletonIssues"><div><span><i className="skBadge"/><i className="skBadge pale"/></span><i className="skText medium"/><i className="skText"/><i className="skText short"/></div><div><span><i className="skBadge"/><i className="skBadge pale"/></span><i className="skText short"/><i className="skText"/><i className="skText medium"/></div></div><div className="skeletonApproach"><i className="skLabel"/><i className="skText medium"/><i className="skText"/><i className="skText short"/></div></div>:null}
