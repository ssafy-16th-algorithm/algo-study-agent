  ...
  return <div className="appShell"><SiteHeader/><main className="detailMain">
    ...
    {detail?<>
      ...
      {selected?.code?<section className="workspace">
        ...
        <div className="reviewCallout"><div><strong>이 풀이를 한 번 더 점검해 볼까요?</strong><span>버튼을 누르면 AI 리뷰를 요청합니다.</span></div><button className="reviewTrigger" type="button" disabled={reviewStatus==='loading'} onClick={()=>void requestReview()}>{reviewStatus==='loading'?'코드 분석 중…':review?'AI 리뷰 다시 보기':'AI 리뷰 받기'} <span>→</span></button></div>
        ...
        <section className="aiReview">
          ...
          {review?<div className="reviewBody"><div className="reviewSummary"><span>우선순위</span><h2>{review.verdict}</h2><p><strong>복잡도</strong> {review.complexity}</p></div>
            <div className="issueList">{review.issues.map((issue,index)=><article className="issueCard" key={issue.line+'-'+index}><div className="issueMeta"><span className={`issueKind kind${index%4}`}>{issue.kind}</span><small>LINE {issue.line}</small></div><h3>{issue.title}</h3><p><strong>근거</strong>{issue.evidence}</p><p><strong>영향</strong>{issue.impact}</p><p className="suggestion"><strong>수정</strong>{issue.suggestion}</p>{issue.codeExample?<div className="reviewCode"><div className="reviewCodeHeader"><span>JAVA</span><strong>수정 예시</strong></div><pre tabIndex={0}><code>{issue.codeExample}</code></pre></div>:null}</article>)}</div>
            <article className="approachCard"><span>더 나은 접근</span><h3>{review.betterApproach.title}</h3><ol>{review.betterApproach.steps.map((step)=><li key={step}>{step}</li>)}</ol><p>{review.betterApproach.complexity}</p></article><div className="testCase"><strong>검증할 반례</strong><p>{review.testCase}</p></div>
          </div>:null}
          {reviewStatus==='error'?<div className="reviewFallback"><strong>AI 리뷰를 완료하지 못했습니다.</strong><p>{reviewError}</p><button className="primaryButton" type="button" onClick={()=>void requestReview()}>다시 요청하기</button></div>:null}
