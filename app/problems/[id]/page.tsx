      ...
      {selected?.code?<section className="workspace">
        ...
        <section className="aiReview">
          ...
          {review?<div className="reviewBody"><div className="reviewSummary"><span>한 줄 평가</span><h2>{review.verdict}</h2><p><strong>복잡도</strong> {review.complexity}</p></div>
            ...
            {review.issues.length?<div className="issueList">{review.issues.map((issue,index)=><article className="issueCard" key={issue.line+'-'+index}><div className="issueMeta"><span className="issueLabels"><em className={`severity severity${issue.severity==='반드시 수정'?'Must':issue.severity==='선택 사항'?'Optional':'Improve'}`}>{issue.severity}</em><span className={`issueKind kind${index%4}`}>{issue.kind}</span></span><small>LINE {issue.line}</small></div><h3>{issue.title}</h3><p><strong>근거</strong>{issue.evidence}</p><p><strong>영향</strong>{issue.impact}</p><p className="suggestion"><strong>수정</strong>{issue.suggestion}</p>{issue.codeExample?<div className="reviewCode"><div className="reviewCodeHeader"><span>JAVA</span><strong>최소 수정 예시</strong></div><pre tabIndex={0}><code>{issue.codeExample}</code></pre></div>:null}</article>)}</div>:<div className="reviewNoIssues"><strong>현재 풀이에서 반드시 고쳐야 할 문제는 찾지 못했습니다.</strong><p>정답성과 성능이 충분하다면 다른 알고리즘으로 억지로 바꿀 필요가 없습니다.</p></div>}
